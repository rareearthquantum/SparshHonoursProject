...
# ╔═╡ e712f3aa-7766-4ff0-8cdf-5521eb299508
using Plots, Measures, OrdinaryDiffEq, LinearAlgebra, Interpolations, FFTW, LaTeXStrings, PlutoUI

# ╔═╡ 0ee10973-e486-4fbd-9fca-1adf26a6538d
md"""
## Stepping Methods
"""

# ╔═╡ 74a52f44-aba2-452f-b3bc-80f7d3ecc4c0
function euler_step!(v,f,h,i)
	v[i+1] = v[i] + h*f(i)
end

# ╔═╡ 531de10a-80a6-4d14-8fe1-95fb827073b4
...
# ╔═╡ 9497b76d-af14-4e22-8c7c-a22a05f28887
...
# ╔═╡ a78e498f-29bf-4531-ae4d-1b13a178215e
...
# ╔═╡ 18bc2d84-aaad-4d92-b194-f0602dfe5cad
...
# ╔═╡ 13848456-a63d-49c1-817d-722143a43590
md"""
## Input Pulse Methods
"""

# ╔═╡ b30830b9-f4a8-4df9-8bab-f02007de85c8
function pulse(
	t::Float64,
	t0::Float64,
	width::Float64,
...
# ╔═╡ 6d17fbf7-b397-4df5-8ac8-84977d15387a
...
# ╔═╡ 4a3813b4-f8b4-4e1e-a26c-540c544157b9
...
# ╔═╡ 05b27ce3-d720-438a-b69a-0df0b2e26eb4
md"""
## Differential Equations
"""

# ╔═╡ 2f784238-e6e9-429f-9d8a-7f3812e99cf7
...
# ╔═╡ 4b8ca47b-2d34-49ff-a6d9-131f948cf798
f_a_2d(
	α::Float64,
	s_grid::AbstractArray,
	konstant::Float64,
	dy2::ComplexF64
) = im*(α*sum(s_grid) + konstant*dy2)

# ╔═╡ 2192ddee-34a2-455d-a45b-3163f901ca11
md"""
## Implementation
"""

# ╔═╡ fdd6779b-34a2-4cbf-ba1c-94ec29bc9b41
...
# ╔═╡ afe04c69-7589-49cf-82fe-9ad87bf05c5a
function stepping_a_4step!(v::AbstractArray,f::Function,h::Float64,i::Int64)
	if (i >= 4)
...
# ╔═╡ f53f13e6-5281-498b-b4dd-05d192f6521f
function stepping_a_3step!(v::AbstractArray,f::Function,h::Float64,i::Int64)
	if (i >= 3)
		adams_bashforth_3step!(v, f, h, i)
	else
		if (i==1)
...
# ╔═╡ 365c29c9-a2ce-40de-a1d3-7a626a699dc2
function stepping_a_2step!(v::AbstractArray,f::Function,h::Float64,i::Int64)
	if (i >= 2)
		adams_bashforth_2step!(v, f, h, i)
	else
		euler_step!(v, f, h, i)
...
# ╔═╡ fc1987d9-8415-4a16-a348-03ebc3532eeb
...
# ╔═╡ c3f83928-5a2b-11f1-a7b3-ff05f209adb2
function evolve_diff_2d(
	...
	(z_grid,t_grid,d_grid,y_grid,s_grid,a_grid), 
		(dz,dt,dy), 
	...
	for i in 1:Nz-1
		for l in eachindex(y_grid)
			...
			prob = 
				ODEProblem(s_evolve!,s_grid[i,:,1,l],(Ti,Tf),(d_grid[i,:,l],Nd,LinearInterpolation(t_grid,a_grid[i,:,l])))
			
			s_grid[i,:,:,l] .= 
				solve(prob,Tsit5(),reltol=1e-3,abstol=1e-6,saveat=t_grid)
...
# ╔═╡ b5b2b432-4f2b-4bd1-bfec-ee9b90f57415
md"""
# Running
"""

# ╔═╡ fed83546-5fa4-4a0d-a60c-282a24a3d8f3
begin ##INPUT PARAMS
...
# ╔═╡ 41a33903-3acc-429a-9eb6-3ebcf7d446c0
...
# ╔═╡ 7f23a988-1fe3-4a36-ad02-485eb2adb470
heatmap(new_y_grid, new_z_grid, abs2.(new_a_grid[:,Nt÷2,:]),c=:viridis,
		...
		clims=(0.0,1.0),size=(700,500))

# ╔═╡ 00f050ae-321e-4efc-a53f-6ee60f486780
heatmap(new_t_grid, new_z_grid, abs2.(new_a_grid[:,:,Ny÷2]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="\nElectric field against t and z",
	   left_margin=5mm, right_margin=10mm, top_margin=5mm, bottom_margin=5mm, clims=(0.0,1.0))

# ╔═╡ 280ed046-235e-41ea-98ec-1a76ac5f45c1
md"""
### FFT Y-domain
"""

# ╔═╡ fa3e1a82-6e92-4ca2-96ed-a3042ca9ed0e
fft_v_a_grid = fftshift(fft(new_a_grid[:,Nt÷2,:],2))

# ╔═╡ 62abb64f-bb0e-425e-8f74-6ceed68d0157
fft_y_grid = fftshift(fftfreq(Ny,Ny/(Yf-Yi)))

# ╔═╡ 0ba10d20-a15d-4f2d-9eb2-c877ced1c5e4
heatmap(fft_y_grid, new_z_grid, abs2.(fft_v_a_grid[:,:]),c=:viridis,
		xlabel="v", ylabel="z", title="\nElectric field intensity against v and z",
		left_margin=5mm, right_margin=10mm, top_margin=5mm, bottom_margin=5mm,
		clims=(0.0,1.0),size=(700,500))

# ╔═╡ 6356f3e8-6437-4166-af04-2ec3ec8a7ae6
md"""
## Animation
"""

# ╔═╡ b04a205b-fdd6-459b-b6c2-3b19c7095dbc
# ╠═╡ disabled = true
#=╠═╡
let
	default(c=:viridis, 
	...
	@gif for i in 1:Nt
		heatmap(new_y_grid, new_z_grid, abs2.(new_a_grid[:,i,:]))
	end every 2
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "76c00830d66e9446840db168ac6f37e87d57d32e"
