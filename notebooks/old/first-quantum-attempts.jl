...
# ╔═╡ e1213dba-2bd7-11f1-94d6-1fe0486052df
using Plots

# ╔═╡ 01ec08bb-3f2a-48f1-9a6e-8c34e3cfd81b
using OrdinaryDiffEq

# ╔═╡ d945a241-6a42-4449-84b5-41e0cc417ad6
import QuantumToolbox as QT

# ╔═╡ beab4047-1c46-4f2c-a29f-694e02051950
import LinearAlgebra as LA

# ╔═╡ 952b30d5-0028-4849-95f1-93d18f9a1717
# ╠═╡ disabled = true
#=╠═╡
md"""
## Humble Beginnings
...
# ╔═╡ 3832fdf2-9906-4e1e-bda6-e463dcab6f08
# ╠═╡ disabled = true
#=╠═╡
let
	rx = 0
	ry = 0
	rz = 1.0
	densitymatrix = 0.5*(QT.one(QT.sigmaz()) + rx*QT.sigmax() + ry*QT.sigmay() + rz*QT.sigmaz())

	function expectation_val(operator::QT.QuantumObject)
		return QT.tr(densitymatrix*operator)
	end
...
# ╔═╡ a62bf0d6-2004-405b-8462-53477eafefd5
# ╠═╡ disabled = true
#=╠═╡
md"""
## 🛑 The below is my attempt to copy the MaxwellBloch python package into Julia
"""
  ╠═╡ =#

# ╔═╡ 849804a8-d0d6-48c8-95cd-fc8972c3f958
# ╠═╡ disabled = true
#=╠═╡
function sigma(n, a, b)
	return QT.basis(n, a) * QT.basis(n,b)'
end
  ╠═╡ =#

# ╔═╡ c99cba83-f0a6-4dd4-b481-ee215253ab43
# ╠═╡ disabled = true
#=╠═╡
sigma(2,1,1)
  ╠═╡ =#

# ╔═╡ 5543ce38-74af-4181-adb1-7b7c984f8bf9
# ╠═╡ disabled = true
#=╠═╡
function sigma_N(n, a, b, i, N)
	sigma_list = Array{QT.QuantumObject, N}
	for j in 1:N
		sigma_list[j] = QT.identity(n)
	end
	sigma_list[i] = sigma(n,a,b)
	return QT.tensor(sigma_list)
end
  ╠═╡ =#

# ╔═╡ 1b51a4e4-36ad-4a8f-8454-a03d5788342c
# ╠═╡ disabled = true
#=╠═╡
Base.@kwdef struct OBBase
	#=
	...
	n_states::UInt8 = 2
	ρ::QT.QuantumObject #density matrix
		
	H_0::QT.QuantumObject #bare hamiltonian
	collapse_operators::Vector{QT.QuantumObject}

	H_Δ::QT.QuantumObject #hamiltonian for detunings
	H_Ω_list::Vector{QT.QuantumObject} #list of hamiltonians for rabi frequencies

	result::QT.TimeEvolutionSol
end
  ╠═╡ =#

# ╔═╡ b05b8c53-5102-4fdd-9914-cc686415b52b
# ╠═╡ disabled = true
#=╠═╡
function set_H_0(object::OBBase, energies::Vector{Float64})
	if energies |> isempty
		H_0 = LA.zeros(object.n_states, object.n_states)
	else
		H_0 = LA.diagm(energies)
	end
	return QT.QuantumObject(H_0)
end
  ╠═╡ =#

# ╔═╡ f5f1db2d-d7c4-4cad-b812-50f133cf7f88
# ╠═╡ disabled = true
#=╠═╡
function H_I_list(object::OBBase)
	return object.H_Ω_list
end
  ╠═╡ =#

# ╔═╡ 25c8b22e-7df1-45c7-9184-3bc422a51fe1
# ╠═╡ disabled = true
#=╠═╡
function H_I_sum(object::OBBase)
	H_I = QT.QuantumObject(LA.zeros(object.n_states, object.n_states))
	for i in object |> H_I_list
		H_I += i
	end
	return H_I
end
...
# ╔═╡ 6d0c74a1-bf04-4f31-a5f0-1e34b2f9fa2a
# ╠═╡ disabled = true
#=╠═╡
function states_t(object::OBBase)
	if object.collapse_operators |> isempty
		states_t = LA.zeros(Complex, (length(object.result.times), object.n_states, 1))
	else
		states_t = LA.zeros(Complex, (length(object.result.times), object.n_states, object.n_states))
	end

	for (t, state_t) in enumerate(object.result.states)
...
# ╔═╡ c16075d7-e48f-455b-80ab-afa13f3434ab
md"""
# AAAAAAAAAAA
"""

# ╔═╡ ef9dc75d-d4d3-4bd9-b212-346327d751b5
begin
	Ns = 2 #number of states, 2 for a 2 level system
	Nz = 128 #number of z-sites
	Nd = 64 #number of detunings/atoms at each z-site
	Nt = 128 #number of time steps
end

# ╔═╡ e414c147-4f60-49e5-97c7-05511d71ab33
begin
	rx = 0.0
	ry = 0.0
	rz = 1.0
	densitymatrix_ini = 0.5*(QT.one(QT.sigmaz()) + rx*QT.sigmax() + ry*QT.sigmay() + rz*QT.sigmaz())
end

# ╔═╡ f7d475ed-8e15-4cc5-8945-e8792728e0bc
begin #initialising all grids

	#have to manually initialise all elements of the operator grids
	sigmam_grid = Array{QT.QuantumObject}(undef,Nz, Nd, Nt)
	for i in 1:Nz
		for j in 1:Nd
			for k in 1:Nt
				sigmam_grid[i,j,k] = QT.zero(QT.sigmaz())
			end
		end
	end
	aop_grid = Array{QT.QuantumObject}(undef,Nz, Nt) #does not depend on detuning
	for i in 1:Nz
		for j in 1:Nt
			aop_grid[i,j] = QT.zero(QT.sigmaz())
		end
	end

	#density matrix
	densitymatrix_grid = Array{QT.QuantumObject}(undef,Nz, Nd, Nt)

	#expectation value grid (might be able to omit as mesolve can output)
	expect_sigmam_grid = Array{ComplexF64}(undef, Nz, Nd, Nt)
	expect_aop_grid = Array{ComplexF64}(undef, Nz, Nt)
	...
	Ti = 0.0
	Tf = 11.0
	time_grid = LinRange(Ti,Tf,Nt)

	#initialising detunings grid
	d_width = 2.0
	d_grid = d_width*randn(Nz, Nd)
end;

# ╔═╡ 626ab7d9-1e1c-4cbc-910a-ded2a588b44f
begin
	dt = (Tf - Ti) / Nt
	dz = (Zf - Zi) / Nz
end

# ╔═╡ 4d59f2aa-7a10-4e98-b371-a1624119899d
begin
	α = 1.0
	β = 1.0
	γ = 1.0
...
# ╔═╡ fbac0f26-68b2-4b31-ac51-e3af7a4b11bd
# ╠═╡ disabled = true
#=╠═╡
hamiltonian = α*QT.sigmaz() + β*QT.num(Ns) + γ*(QT.destroy(Ns)*QT.sigmap() + QT.create(Ns)*QT.sigmam())
  ╠═╡ =#

# ╔═╡ d3142855-02fd-40c8-bed1-a47b586117d4
# ╠═╡ disabled = true
#=╠═╡
begin

	sigmam_grid[1,1,1] = QT.sigmam()
	aop_grid[1,1] = QT.destroy(Ns)
	
	for i in 1:Nz-1
		for j in 1:Nd
			#solve for density matrix of each atom
			solve = QT.mesolve(hamiltonian,densitymatrix_ini,time_grid,[], saveat=time_grid)
			densitymatrix_grid[i, j, :] = solve.states

			#calculate and save expectation values
			expect_sigmam_grid[i,j,:] .= LA.tr.(densitymatrix_grid[i,j,:].*sigmam_grid[i,j,:])
			expect_aop_grid[i,:] .= LA.tr.(densitymatrix_grid[i,j,:].*aop_grid[i,:])
		end
		for k in 1:Nt-1
			for j in 1:Nd
				#evolving sigma minus (polarisation?)
				sigmam_grid[i,j,k+1] = sigmam_grid[i,j,k] + dt*(-im*d_grid[i,j]*sigmam_grid[i,j,k] + 
					im*aop_grid[i,k])
			end
			#evolving electric field operator
			aop_grid[i+1,k] = aop_grid[i,k] + dz*(im*α*sum(sigmam_grid[i,:,k]))
		end
	end
...
# ╔═╡ 6c68580d-b841-477d-8e64-4e9a3b517d1b
begin
	hams_whole = Array{typeof(QT.sigmaz())}(undef, Nz, Nd, Nt);
	for i in 1:Nd
...
# ╔═╡ ff515f03-585b-4f68-b7e1-04db6c0c642b
# ╠═╡ disabled = true
#=╠═╡
plot(time_grid,[real.(expect_sigmam_grid[1,1,:]),imag.(expect_sigmam_grid[1,1,:])])
  ╠═╡ =#

# ╔═╡ f8963b31-4046-4a19-b9df-424b507cec24
# ╠═╡ disabled = true
#=╠═╡
heatmap(time_grid, z_grid, real.(expect_sigmam_grid[:,1,:]))
  ╠═╡ =#

# ╔═╡ b9fb850c-f084-47b2-a129-6bac6d473fdb
# ╠═╡ disabled = true
#=╠═╡
heatmap(time_grid, z_grid, real.(expect_aop_grid[:,:]))
  ╠═╡ =#

# ╔═╡ 222f8a61-fdf3-4610-a2dc-47e30e3a4c58
md"""
# AAAA back to semiclassical
"""

# ╔═╡ ac903da5-edf9-4cc8-a021-42c8246ab3f9
function pulse(t,t0,width,A)
	A * exp(-((t-t0)/width)^2)
end

# ╔═╡ 03f3900e-cbd6-4b43-bc79-fc5c1902fcc5
function Ein(t)
	p1 = pulse(t,5.0,1.0,1.0)
	#p2 = pulse(t,18.0,1.0,1.0)
	return p1
end

# ╔═╡ ab96396b-74b6-4790-9f11-e3c118738a29
function my_rk4_term(f, h, i)
	return h/6 * (f(i) + 4*f(i+1) + f(i+2))
end

# ╔═╡ 25359acf-fbed-4ec4-ab42-cb3c34ad2157
# ╠═╡ disabled = true
#=╠═╡
function f1!(du, u, p, t) #vary in t, fix z and d
	d_grid, a_grid = p
	du .= im.*d_grid.*u + im.*a_grid
end
  ╠═╡ =#

# ╔═╡ d5468e34-4ff4-4e4e-be47-b5e1b2f21ebc
# ╠═╡ disabled = true
#=╠═╡
function f2!(du, u, p, t) #fix t and sum over detunings, vary in z
	α_c, s_grid = p
	du .= im.*α_c.*reduce(+, s_grid, dims=2) #s_grid passed in should be 2d
end
  ╠═╡ =#

# ╔═╡ 92067c20-c358-49e0-9505-2a97d087cafb
# ╠═╡ disabled = true
#=╠═╡
function f3!(du, u, p, t)
	...
	for j in eachindex(s)
		ds[j] = im*d_val*s[j] + im*a_grid[j]
	end

end
  ╠═╡ =#

# ╔═╡ d67bf287-35b6-441c-bcb0-766fee83a1cc
begin
	s_grid = Array{ComplexF64}(undef, Nz, Nd, Nt)
	a_grid = Array{ComplexF64}(undef, Nz, Nt)
	α_c = 0.01

	s_grid[:,:,1] .= zeros(Nz,Nd)
	a_grid[1,:] .= Ein.(time_grid)

	fs1(i,j,k) = im*d_grid[i,j]*s_grid[i,j,k] + im*a_grid[i,k]
	fa1(i,k) = im*α_c*sum(s_grid[i,:,k])
end

# ╔═╡ e2912f00-3b9a-4f05-8a6f-19d80c95bbe7
md"""
## Simple Euler step
"""

# ╔═╡ e99f8a20-e628-443d-b089-e05201d23cc9
# ╠═╡ disabled = true
#=╠═╡
begin	
	for i in 1:Nz-1
		for j in 1:Nd
			for k in 1:Nt-1
				...
				s_grid[i,j,k+1] = s_grid[i,j,k] + dt*(fs1(i,j,k))
			end
		end
		
		#evolving electric field
		for k in 1:Nt
			a_grid[i+1,k] = a_grid[i,k] + dz*(fa1(i,k))
		end
	end
...
# ╔═╡ 25ad1ad0-6291-4967-945d-4165cfe9c356
md"""
## RK4 attempt
"""

# ╔═╡ d7cf9ca2-7467-4269-a58f-5ca1261baf51
begin
	for i in 1:Nz-1
		#evolving polarisation
		for j in 1:Nd
			#euler stepping first couple of steps
			for k in 1:2
				s_grid[i,j,k+1] = s_grid[i,j,k] + dt*(fs1(i,j,k))
			end
			#rk4 the rest
			for k in 1:Nt-3
				s_grid[i,j,k+3] = s_grid[i,j,k] + my_rk4_term(l -> fs1(i,j,l), dt, k)
			end
		end
		#evolving electric field
		#euler stepping
...
# ╔═╡ 5c6328d4-6498-45c5-9e60-6ad1844d4ca7
heatmap(time_grid, z_grid, real.(a_grid[:,:]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="Electric field against time and space")

# ╔═╡ c35e5a52-f3e8-476a-a338-0c2bac8c883f
plot(time_grid,real.(a_grid[end,:]))

# ╔═╡ 18b2195c-1161-4ffe-ab1b-5c6a2466b258
heatmap(time_grid, z_grid, real.(s_grid[:,1,:]), c=:viridis)
...
manifest_format = "2.0"
project_hash = "6437685ea650451a8c18117c3d527728d077416a"
...
[[deps.QuantumToolbox]]
deps = ["ArrayInterface", "DiffEqCallbacks", "DiffEqNoiseProcess", "Distributed", "FFTW", "FillArrays", "Graphs", "IncompleteLU", "LaTeXStrings", "LinearAlgebra", "LinearSolve", "OrdinaryDiffEqCore", "OrdinaryDiffEqLowOrderRK", "OrdinaryDiffEqVerner", "Pkg", "ProgressMeter", "Random", "SciMLBase", "SciMLOperators", "SparseArrays", "SpecialFunctions", "StaticArraysCore", "Statistics", "StochasticDiffEq"]
git-tree-sha1 = "4d73a931c38c267eb60b6d8b8725fa2dbf4e3006"
uuid = "6c2fb7c5-b903-41d2-bc5e-5a7c320b9fab"
version = "0.43.1"

    [deps.QuantumToolbox.extensions]
