...
# ╔═╡ e0d5c3bc-48c9-11f1-ba97-850e7fe822c4
using Plots

# ╔═╡ 7161b937-9c0c-41d6-a7ca-1f50c49597b5
using OrdinaryDiffEq

# ╔═╡ bd35ae71-a040-42a5-a675-da061248299c
using Measures

# ╔═╡ 622505cf-36f6-40f7-94c3-f0e51049b4b5
import LinearAlgebra as LA

# ╔═╡ 1b417411-8609-4cac-9dd5-c790e7df857f
md"""
# 🔴 Initialising Parameters
"""

# ╔═╡ ec9ade4d-b7a4-4f42-9aa7-d4de5c9816a0
begin
...
# ╔═╡ 8c471efa-b68c-4646-b32b-5d021998f5a5
begin
	#initialising z
	Zi = 0.0
	Zf = 10.0
...
# ╔═╡ 942a908a-a482-49fa-a3f7-e17185160ceb
begin
	#initialiasing t
	Ti = 0.0
	Tf = 11.0
...
# ╔═╡ a440914f-952b-4e2b-84fc-2cf1b9e677d3
begin
	dt = (Tf - Ti) / Nt
	dz = (Zf - Zi) / Nz
end;

# ╔═╡ 1225dcb2-1f1f-4080-9ddd-bb9135ad568c
function pulse(t,t0,width,A)
	A * exp(-((t-t0)/width)^2)
end

# ╔═╡ a9f718bb-61af-41a5-bdca-2d25c4da1935
md"""
Note that the fourier transform of a gaussian of the form A * exp(-((t-t0)/width)^2) is 
"""

# ╔═╡ e5afb90d-c9e0-4f1d-b465-4c53d225fd3d
function Ein(t)
	p1 = pulse(t,5.0,0.01,1.0)
	#p2 = pulse(t,18.0,1.0,1.0)
	return p1
end

# ╔═╡ 55e40e5d-88a7-4d2a-b950-fd7560154c2d
md"""
### Δ
"""

# ╔═╡ 14f82fe0-4bed-471e-aeb4-469910aec61f
begin
	#initialising detunings grid, flat range for each z-site
	d_grid = Array{Float64}(undef,Nz,Nd);

	df = 1/(Nt*dt)
	@show d_width = df*Nt * 1.5
...
# ╔═╡ 6e2e41da-105f-4636-a920-56b67e2c0455
function simpsons_rule(f, h, i)
	return h/6 * (f(i) + 4*f(i+1) + f(i+2))
end

# ╔═╡ f88a06bc-b243-4cb6-9220-96affa9cdd9a
begin
	s_grid = Array{ComplexF64}(undef, Nz, Nd, Nt)
	sum_s_grid = Array{ComplexF64}(undef,Nz,Nt)
	...
	a_grid[1,:] .= Ein.(t_grid)
	sum_s_grid[:,1] = zeros(Nz)

end;

# ╔═╡ cdc99a19-d956-48ac-a5d0-5bc1d09f2853
function adams_bashford_2step(f,i)
	if i == 1
		f(i)
	elseif i >= 2
		return (3/2)*f(i+1) - (1/2)*f(i)
	end
end

# ╔═╡ 39ceec9d-2e9e-4915-87c7-d1f217457adf
function adams_bashford_2step!(v,f,h,i,N)
	if i >= N-2
		return
	end
	...
	if i == 1
		v[i+1] = v[i] + h*f(i)
	elseif i >= 2
		v[i+2] = v[i] + h*((3/2)*f(i+1) - (1/2)*f(i))
	end
end

# ╔═╡ 2c793841-9549-4b78-a56c-4e4e4d901696
function euler_step!(v,f,h,i,N)
	if i >= N-1
		return
	end
...
# ╔═╡ 52598b42-4676-476e-93d6-7f7d7555c4f4
function s_evolve!(du,u,p,t)
	
	d,Nd, a_func = p
	
	du .= im.*d.*u + im.*ones(Nd).*a_func(t)
end

# ╔═╡ 9b1cfaf7-9336-4ee2-96be-371e954f9692
md"""
# 🔴 Evolution functions
"""

# ╔═╡ d9a4465c-936c-4a2e-b3b6-82ad297c4e6b
fs1(i,j,k) = im*d_grid[i,j]*s_grid[i,j,k] + im*a_grid[i,k]

# ╔═╡ 8f588082-1c59-4f97-b2c8-fe9b5d49220c
fa1(i,k) = im*α*sum(s_grid[i,:,k])

# ╔═╡ 0dcacd52-9505-4220-98c8-3727c4faef3f
function evolve_euler!(grids,differentials,N,p)
...
# ╔═╡ 47a9af0b-f2c5-4d3e-80df-78d645e98b07
function evolve_euler2!(grids,differentials,N,p)
	...
	for i in 1:Nz
		...
		for j in 1:Nd
			for k in 1:Nt
				s_temp = @view s_grid[i,j,:]
				f = l->fs1(i,j,l)
				euler_step!(s_temp, f, dt, k, Nt)
			end
		end
...
# ╔═╡ da023b53-a8aa-44a0-9a9b-c8e7b9fcdbcf
function evolve_diff!(grids,differentials,N,p)
	...
	for i in 1:Nz
		...
		prob = ODEProblem(s_evolve!,s0,(Ti,Tf),(d_grid[i,:],Nd,Ein))
		sol = solve(prob,Vern6(), reltol=1e-9, abstol=1e-9,saveat=t_grid)
...
# ╔═╡ ef26fc69-7e3e-4ffe-be3f-02bbfaa73f30
function evolve_rk4_proto!(grids,differentials,N)
	...
	for i in 1:Nz-3
		...
		#euler stepping
		for k in 1:Nt
			a_grid[i+3,k] = a_grid[i,k] + my_rk4_term(l -> fa1(l,k), dz, i)
		end
	end
	
end

# ╔═╡ 0d4f4738-5b03-43c8-b0b0-59bea32d16a4
function evolve_euler_proto!(grids,differentials,N,p)

	sum_s_grid, a_grid = grids
	dt, dz = differentials
	Nz,Nd,Nt = N
	α = p

	s_grid = zeros(ComplexF64, Nz, Nd)

	@show u_grid = vcat(sum_s_grid,a_grid)
	
	for k in 1:Nt-1
		for i in 1:Nz-1
			s_grid[i,:] .= s_grid[i,:] + dt.*(fs1.(i,1:Nd,k))
			u_grid[i, k+1] = sum(s_grid[i,:])
			u_grid[i+1, k] = a_grid[i,k] + dz*(fa2(α,sum_s_grid[[i,k]]))
		end
	end
	
end

# ╔═╡ 03e3e1c0-2f01-453a-a207-6bbeca2e8586
evolve_diff!((s_grid,a_grid),(dt,dz),(Nz,Nd,Nt),(α))

# ╔═╡ cda2d9de-a14b-4d12-b9e0-06771f41b579
md"""
# 🔴 Plots
"""

# ╔═╡ 61d61172-f6ca-45b6-b87d-67c2a3f8bf9b
heatmap(t_grid, z_grid, real.(a_grid[:,:]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="Electric field against time and space",
	   right_margin=10*mm)

# ╔═╡ 61bcc033-f637-4668-82ff-0a4571630f92
import FFTW as FFT

# ╔═╡ 1f2daf43-622f-497c-98d0-fdc15ffd9325
let
	fs = 1000
	T = 10.0
	@show t = 0.0:1/fs:T-1/fs
	n = length(t)
	...
	F = FFT.fft(signal)
	@show freqs = FFT.fftfreq(Nt,fs2)

	F_shifted = FFT.fftshift(F)
	freqs_shifted = FFT.fftshift(freqs)

	plot(freqs_shifted, abs.(F_shifted))
...
manifest_format = "2.0"
project_hash = "582b8c74034becb46d5f2338e27a0880fcab73b2"
