...
# ╔═╡ 4e8643c2-4aa1-11f1-b384-633abdc21e3b
using Plots, Measures, OrdinaryDiffEq, LinearAlgebra, Interpolations, FFTW, LaTeXStrings

# ╔═╡ 9353c599-e5b8-40a6-98df-b9ee05f61654
using FLoops

# ╔═╡ d74ef3f2-d57a-40ee-9232-985a063a9b91
using PlutoUI

# ╔═╡ c6489167-07a1-4c6f-ba0a-46e3a265c4bc
using CairoMakie

# ╔═╡ d1761e3a-bfae-49c7-b7d8-00b14961ec52
md"""
# 🌐 Initialising grids
"""

# ╔═╡ cf50e644-1bc0-46d2-a4c6-a7f250e810ad
begin
...
# ╔═╡ bbad6e5a-6fb2-4f7c-a00c-2ee0ae28bac9
begin
	#initialising z
	Zi = 0.0
	Zf = 10.0
...
# ╔═╡ 9f0515ea-c4e3-433c-aea8-543b03e0284b
begin
	#initialiasing t
	Ti = 0.0
	Tf = 10.0
...
# ╔═╡ aeff8512-e903-4032-8435-b4e88c02aac6
begin
	#initialising detunings grid, flat range for each z-site
	d_grid = Array{Float64}(undef,Nz,Nd);

	@show df = 1/(Nt*dt)
	d_width = df*Nt
	
	for i in 1:Nz
		d_grid[i,:] = LinRange(-d_width,d_width,Nd)
	end

	### NOTE, need to base this off of pulse width rather than time grid (i think)
end

# ╔═╡ 2fab7834-2f4a-4bf6-9951-b349b2761d67
α = 1.0e-1

# ╔═╡ bc8eacea-9bde-4992-8d55-5f278efcdd86
md"""
## 🔧 Initialising some functions
"""

# ╔═╡ a56e3756-6eeb-40f6-b4af-5fa65ae525ec
md"""
### - Little funcs
"""

# ╔═╡ 2f4bd1ed-3296-40a4-9b49-76b26726cb01
function pulse(
	t::Float64,
	t0::Float64,
	width::Float64,
...
# ╔═╡ 7adaf0dc-4af6-4991-9d69-99762d159f67
...
# ╔═╡ a3cb40fd-823b-4314-9b06-b6c9a20a69ad
begin
	s_grid = Array{ComplexF64}(undef, Nz, Nd, Nt)
	s_grid[:,:,1] .= zeros(Nz,Nd)

	a_grid = Array{ComplexF64}(undef, Nz, Nt)
	a_grid[1,:] .= Ein_t.(t_grid)
end;

# ╔═╡ 9498da08-d815-45bc-9ba4-d3f043f31f08
function adams_bashford_2step!(
	v::AbstractArray,
	f::Function,
	h::Float64,
	i::Int64,
	...
	#original adams-bashford: v[i+2] = v[i+1] + h*(3/2 f(i+1) - 1/2 f(i))
	(i >= 2) && (v[i+1] = v[i] + h*((3/2)*f(i) - (1/2)*f(i-1))) #shift i down by 1
end

# ╔═╡ a0ec76e5-c2b6-487e-b9bd-6324619802a1
md"""
### - Individual evolution funcs
"""

# ╔═╡ f7b70eeb-7466-4f44-990e-6ef6df51d130
...
# ╔═╡ a6850b3c-67ad-46b1-8d9f-82e074bbc8e1
a_diffeq_func(i,k) = im*α*sum(sm_grid[i,:,k])

# ╔═╡ 87d903e1-b78d-44ae-806e-63e333a0e51b
f_a(α::Float64,grid::AbstractArray) = im*α*sum(grid)

# ╔═╡ aebb1d0b-7aae-45d7-a098-78bb33edb989
function a_evolve!(du,u,p,t)
	
	α,s_grid = p
	
	du .= im.*α.*dropdims(sum(s_grid,dims=1),dims=1)
end

# ╔═╡ 5783d060-90ca-4fb2-8295-69eab7a5ce8f
md"""
### - Evolution schemes
"""

# ╔═╡ d1cfede4-4fcf-45ab-8640-13020d81f44e
function evolve_diff!(grids,p)

	s_grid, a_grid, d_grid, z_grid, t_grid = grids
	α, N = p
	
	Nz,Nd,Nt = N
	dz = (z_grid[end] - z_grid[begin])/Nz

	Ti, Tf = t_grid[begin], t_grid[end]

	a_interps = Array{AbstractInterpolation}(undef,Nz)
	
	for i in 1:Nz-1

		a_interps[i] = LinearInterpolation(t_grid,a_grid[i,:])
		#@show a_interps[i].(t_grid[end])
		
		#evolving polarisation
		s0 = s_grid[i,:,1]

		prob = ODEProblem(s_evolve!,s0,(Ti,Tf),(d_grid[i,:],Nd,a_interps[i]))
		sol = solve(prob,Tsit5(),reltol=1e-3,abstol=1e-6,saveat=t_grid)
		...
		for k in eachindex(t_grid)
			#a_temp = @view a_grid[:,k]
			#f = l->f_a(α,s_grid[i,:,k])
			#adams_bashford_2step!(a_temp, f, dz, i, Nz)
		...
		zi = z_grid[i]
		zf = z_grid[i+1]
		prob_a = ODEProblem(a_evolve!,a_grid[i,:],(zi,zf),(α,s_grid[i,:,:]))
		sol_a = solve(prob_a,Tsit5(),reltol=1e-3,abstol=1e-6,saveat=[zi,zf])
		a_grid[i+1,:] .= sol[2,:]
...
# ╔═╡ e62b22ec-7913-4e08-be17-a06d71ef5747
function evolve_diff_make(grids,p)
	Nz = 128 #number of z-sites
	Nd = 64 #number of detunings/atoms at each z-site
	Nt = 128*2 #number of time steps
	...
	for i in 1:Nz
		...
		for k in 1:Nt
			a_temp = @view a_grid[:,k]
			f = l->a_diffeq_func(l,k)
			adams_bashford_2step!(a_temp, f, dz, i, Nz)
		end
		#@show a_grid[i,end]
	end

	return (z_grid, t_grid, s_grid, a_grid)
end

# ╔═╡ b4a47270-c122-4e5e-9494-48d3ff23db50
md"""
# 🔆 Solving
"""

# ╔═╡ f5701580-1e40-4f95-a0fe-b4c65f66f083
evolve_diff!((s_grid,a_grid,d_grid,z_grid,t_grid),(α,(Nz,Nd,Nt)))
# AAAAAAAAAAAAAAAA, the grids never reinitialise, so it dies if u change stuff

# ╔═╡ dde743c0-594f-4153-9a7c-e2ec7892f3b6
md"""
# 📊 Plotting
"""

# ╔═╡ 67d8aa21-247e-40ee-b948-a60a59580498
begin
	pa1 = Plots.heatmap(t_grid, z_grid, real.(a_grid), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="\nElectric field against time and space",
	   left_margin=5mm, right_margin=10mm, top_margin=5mm, bottom_margin=5mm)
end

# ╔═╡ d7b6c1a9-51fe-437a-b7b2-fd708b1b626b
begin
	ps1 = Plots.heatmap(t_grid, z_grid, real.(s_grid[:,begin,:]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="d_grid[begin]",
		right_margin=10*mm)
	ps2 = Plots.heatmap(t_grid, z_grid, real.(s_grid[:,end÷2,:]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="d_grid[end÷2]",
		right_margin=10*mm)
	ps3 = Plots.heatmap(t_grid, z_grid, real.(s_grid[:,end,:]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="d_grid[end]",
		right_margin=10*mm)
	Plots.plot(ps1,ps2,ps3, size=(1080,720), title="lol")
end

# ╔═╡ 50fa0ce0-61a3-4d93-86c5-7d1e937dd22c
md"""
## 📶 Testing FFTW package
"""

# ╔═╡ 75a6170f-b0d4-46ba-b94a-efb77ab82fc1
let
	fs = 1/(dt)
	...
	pulsewidth = 0.1
	pulseamplitude = 1.0
	signal = pulse.(t_grid,pulsecenter,pulsewidth,pulseamplitude)
	...
	Plots.plot(freqs_shifted, abs.(F_shifted))
	
end

# ╔═╡ 77c7c5cd-980d-4f88-9967-9a87fb713870
md"""
# NEW 2D
"""

# ╔═╡ 059049f5-772a-4da0-a14f-63a97fe3d3ee
function Ein_y(y::Float64)
	p_c, p_w, p_A = 0.0, 1.0e-1, 1.0
	p = pulse(y,p_c,p_w,p_A)
	return p
end

# ╔═╡ 2c363897-d275-4452-85e1-e5753ec78cbd
f_a_2d(
	α::Float64,
	s_grid::AbstractArray,
	konstant::Float64,
	dy2::ComplexF64
) = im*α*sum(s_grid) + im*konstant*dy2

# ╔═╡ cf8a0dcc-95cd-453c-8e74-9e3c949e7a39
function initialise_params(
	...
	a_interps = Array{AbstractInterpolation}(undef,Nz,Ny)
	...
	grids = (z_grid,t_grid,d_grid,y_grid,s_grid,a_grid,a_interps)
...
# ╔═╡ 2c4f68fc-64e4-4362-bbc2-3ec3016bee53
function evolving()
	
end

# ╔═╡ 5bfc90d2-02e0-420d-b47e-b706e5a86279
function evolve_a_grid(
	...
	for k in eachindex(t_grid)
		...
		f_2d = x->f_a_2d(α,s_grid[x,:,k,l],konstant,dy2(x))
		adams_bashford_2step!(a_temp, f_2d, dz, i, Nz)
	end
end

# ╔═╡ 22d89033-3cf0-46dc-a035-7dd464a0434e
function evolve_diff_2d(
	...
	(z_grid,t_grid,d_grid,y_grid,s_grid,a_grid,a_interps), 
		(dz,dt,dy), 
		(v_freq,v_factor) = initialise_params(f,N,B)
	
	α::Float64, konstant::Float64 = p
	
	for i in eachindex(z_grid)
		for l in eachindex(y_grid)

			a_interps[i,l] = LinearInterpolation(t_grid,a_grid[i,:,l])
			
			#evolving polarisation
			s0 = @view s_grid[i,:,1,l]
			prob = ODEProblem(s_evolve!,s0,(Ti,Tf),(d_grid[i,:,l],Nd,a_interps[i,l]))
			sol = solve(prob,Tsit5(),reltol=1e-3,abstol=1e-6,saveat=t_grid)
			s_grid[i,:,:,l] .= sol[:,:]
			
			#evolving electric field
			evolve_a_grid(t_grid,a_grid,v_factor,l,α,s_grid,konstant,dz,i,Nz)
		end
	end
...
# ╔═╡ 3fa12109-e830-4ec5-8083-b340f2187e8d
Ny = Nz÷4

# ╔═╡ 2f6f7a05-de41-4cf6-9e6b-628aae40cf2e
y_width = 1.0

# ╔═╡ ee4fb60f-47ba-4472-99dc-f285ed62483e
(new_s_grid, new_a_grid),
	(new_z_grid,new_t_grid,new_y_grid) = evolve_diff_2d((Ein_t,Ein_y), 
										 (Nz,Nd,Nt,Ny), 
										 (
											 (Zi,Zf),
											 (Ti,Tf),
...
# ╔═╡ c21aa8dd-a334-498d-9eaf-ef21ab85b695
Plots.heatmap(new_t_grid, new_z_grid, real.(new_a_grid[:,:,Ny÷2]), 
		c=:viridis, 
		xlabel="t", ylabel="z", title="\nElectric field against t and z",
	   left_margin=5mm, right_margin=10mm, top_margin=5mm, bottom_margin=5mm, clims=(-1.0,1.0))

# ╔═╡ 708357aa-dd4f-4fb2-92dd-b648f28d365c
let
	grid = @view new_a_grid[:,t_slider,:]
	Plots.heatmap(new_y_grid, new_z_grid, abs2.(grid), 
		c=:viridis, 
		xlabel="y", ylabel="z", title="\nElectric field intensity against y and z",
	   left_margin=5mm, right_margin=10mm, top_margin=5mm, bottom_margin=5mm, clims=(0.0,1.0),size=(700,500))

end

# ╔═╡ 0c5c1a48-27ea-4c9a-87e0-5684f256524c
@bind t_slider PlutoUI.Slider(1:Nt,show_value=true)

# ╔═╡ bc20cbc5-8503-44ed-a8d4-d4e5be109a92
let
	dy = 2*y_width/Ny
	v_freq = fftfreq(Ny, 1/dy)
	v_factor = -4.0 .* π^2 .* v_freq.^2
	dy2_experi = (x,y) -> (ifft(v_factor.*fft(new_a_grid[x,t_slider,:])))[y]
	grid_experi = stack([dy2_experi.(1:Nz,l) for l in 1:Ny])
	@show grid_experi |> size
	Plots.heatmap(1:Ny,1:Nz,real.(grid_experi),right_margin=15mm,c=:viridis)
end

# ╔═╡ e5c334e6-6f1d-4234-8958-e40a13a27bf9
md"""
## Makie Experiments
"""

# ╔═╡ c5c55e6a-e5d1-4754-83ad-1b042d9d2754
begin
...
for i in 1:4
    ...
    hm = CairoMakie.heatmap!(ax, datasets[i])
...
manifest_format = "2.0"
project_hash = "72e5d1c3c46880259229a65d7a6abd561dd3c44b"
